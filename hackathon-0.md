...
- Create an accounting system for your business in Odoo Community (self-hosted, local)
...
- Run the AI Employee on Cloud 24/7 (always-on watchers + orchestrator + health
...
- Security rule: Vault sync includes only markdown/state. Secrets never sync (.env,
...
- Deploy Odoo Community on a Cloud VM (24/7) with HTTPS, backups, and health
...
- Platinum demo (minimum passing gate): Email arrives while Local is offline → Cloud
...
## 7.
Think: "I see a WhatsApp message from a client asking for an invoice and a bank
...
- NO → Block exit, re-inject prompt, and allow Claude to see its own previous failed output
(loop continues).
...
- File movement (advanced - Gold tier): Stop hook detects when task file moves to
/Done
...
- Key Hackathon Feature: The "Business Handover"
...
- The Process: Claude Code reads your Business_Goals.md, checks your Tasks/Done
